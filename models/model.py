...
class GCN_LSTM(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        target_node_feature = x[target_node_index]  # [32, gcn_hidden_dim]
...
class GC_TCN(torch.nn.Module):
    def __init__(self, num_node_features, gcn_hidden_dim, tcn_num_channels, num_classes, kernel_size=2, dropout=0.2):
        ...
        # TCN Layer
        self.tcn = TemporalConvNet(num_inputs=gcn_hidden_dim, num_channels=tcn_num_channels, kernel_size=kernel_size, dropout=dropout)
    ...
    def forward(self, data):
        ...
        # GCN layers
        x = F.relu(self.conv1(x, edge_index))
        x = F.relu(self.conv2(x, edge_index))  # [1088, gcn_hidden_dim]
        x = F.dropout(x, p=self.dropout, training=self.training)
