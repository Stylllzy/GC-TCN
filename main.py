...
from config.config import default_target, default_loader_path, default_gcn_hidden_dim, default_lstm_hidden_dim, \
    ...
    default_weight_name, default_savefig_path
...
def main(args):
    ...
    # -------------------------------------------- 3. data -------------------------------------------------------------
    df, test_times = load_data(args.data_path)  # 读取数据
    print(f"文件路径: {args.data_path}，测试集大小: {len(test_times)}，测试集时间: {test_times[0]} - {test_times[-1]}")
    data_normalized, labels_normalized, scaler_data, scaler_labels = normalize_features_and_labels(df,
                                                                                                   args.target)  # 数据标准化
    ...
    if os.path.isfile(model_weights_path):  # 如果存在预训练权重，则加载
        ...
        print('#---------------------------------------------------------------------------------------------------#')
        run_training_loop(model, train_loader, val_loader, optimizer, scheduler, criterion, device, args.num_epochs,
                          model_weights_path)
...
if __name__ == '__main__':
    ...
    # --------------------------------------------  dataset settings ---------------------------------------------------
    parser.add_argument('--data_path', type=str, default='./dataset/tanjiaoyi/hb_carbon.csv', help='which data to use')
    ...
    parser.add_argument('--model', type=str, default=default_model, choices=['gc-fc', 'gc-tcn', 'gcn-lstm', 'gcn-gru'])
